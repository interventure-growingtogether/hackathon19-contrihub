	...
	render() {
		...
		return (
			<Form
				...
				style={{ width: '100%', padding: '25px 25px', background: 'white' }}
			>
				<Row>
					<Col span={8}>
						<ImageUpload
							...
							prefixId={this.uid}
							maxImages={1}
							shouldUpload={this.state.shouldUpload}
						/>
					</Col>
					<Col span={16}>
						...
						<Form.Item validateStatus={descriptionError ? 'error' : ''} help={descriptionError || ''}>
							{getFieldDecorator('description', {
								...
								<Input.TextArea style={{ width: '400px' }} placeholder="Project description" rows={4} />
							)}
						</Form.Item>
						<Form.Item
							label="Technologies"
							validateStatus={techError ? 'technologies' : ''}
							help={techError || ''}
						>
							{getFieldDecorator('technologies', {
								rules: [{ required: true, message: 'Please input technologies!' }],
							})(<CustomInput style={{ width: '180px' }} placeholder="Technologies" />)}
						</Form.Item>
						...
						<Form.Item
							label="Repository"
							validateStatus={repoError ? 'technologies' : ''}
							help={repoError || ''}
						>
							{getFieldDecorator('repo', {
								rules: [{ required: true, message: 'Please input repository!' }],
							})(<CustomInput style={{ width: '180px' }} placeholder="Repository" />)}
						</Form.Item>
						<Form.Item>
							<Button
